...
<div class="container">
    ...
    <table>
        <td>
            <button class="btn btn-primary btn-sm" id="btn_submit" name="btn_submit" style="margin-bottom: 10px; background-color: #0069d9;">อนุมัติ/ยกเลิกการจอง</button>
       </td>
       <td>
            <form onsubmit="return printsub()" method="post" action="/api/v1/report/approved" target="_blank"  style="margin-bottom: 10px;">
                ...
                <input type="hidden" name="inp_bookingID" value="" />
                <button class="btn btn-success btn-sm" type="submit" id="btn_report_app" style="background-color: #28a745;">ใบเสร็จอนุมัติเบิกบัญชี</button>
            </form>
        </td>
    </table>
...
<script  type="text/javascript">
    ...
    function printsub(){
      ...
      var datarow = row_selected('#gridlistline_booking');
      if(!!rowdata){
          $('input[name=inp_app_report]').val(rowdata.MEMBERID);
          $('input[name=inp_bookingID]').val(datarow.BOOKINGID);
          return true;
      }else{
