...
class BookingModel
{
	...
	public function updatestatus($conn)
	{
		...
		$sql1 = "UPDATE BOOKING SET STATUS_CANCEL = 1 WHERE CASE WHEN DATEPART(hour,GETDATE()) - DATEPART(hour,BOOKINGTIME) < 0 
																THEN (DATEPART(hour,GETDATE()) - DATEPART(hour,BOOKINGTIME)) * -1 
																ELSE DATEPART(hour,GETDATE()) - DATEPART(hour,BOOKINGTIME) END >= 6
														OR  DATEDIFF(DAY,BOOKINGDATE,GETDATE()) >= 1";
		if(Sqlsrv::query($conn,$sql1) & Sqlsrv::query($conn,$sql1))
		{
			return true;
		}
		return false;
	...
	public function list($memberid){
		self::updatestatus($this->conn->connect());
		...
		try{
			...
			if(isset($bookingid)){
				$insertbookingbooth = sqlsrv_query(
													...
													,STATUS_OVER
													,BOOKINGTIME
													,STATUS_CANCEL) VALUES(?,?,?,?,?,?,?,1,0,RIGHT(CONVERT(VARCHAR, GETDATE(), 100),7),0)",
													array(
														$bookingid
														,$boothid
														,$memberid
